...
public class EquipeControlador {
	...
	@PostMapping
	public ResponseEntity<Equipe> salvar(@Valid @RequestBody Equipe entidade, HttpServletResponse response) {
		Equipe entidadeSalvo = entidadeBO.salvar(entidade);
	...
	public ResponseEntity<Equipe> buscarPorId(@PathVariable Long id) {
		Equipe entidade = entidadeBO.buscarPorId(id);
	...
	@PutMapping("/{id}")
	public ResponseEntity<Equipe> atualizarPorId(@PathVariable Long id, @Valid @RequestBody Equipe entidade) {
